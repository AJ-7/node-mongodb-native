...
function ChangeStream(changeDomain, pipeline, options) {
  ...
  // We need to get the clusterTime as early as possible
  const isMaster = this.serverConfig.lastIsMaster();
  if (!isMaster) {
    throw new MongoError('ServerConfig does not have an ismaster yet.');
  }

  this.clusterTime = isMaster.$clusterTime && isMaster.$clusterTime.clusterTime;
<<<<<<< HEAD
=======
  const maxWireVersion = isMaster.maxWireVersion || 0;
  if (maxWireVersion >= 7 && !this.clusterTime) {
    throw new MongoError('Cluster time not available from most recent ismaster');
  }
>>>>>>> 271da5840... chore(feedback): incorporating feedback
...
var buildChangeStreamAggregationCommand = function(self) {
  ...
  if (resumeToken || options.resumeAfter) {
    ...
    const ts = options.startAtClusterTime || self.clusterTime;
    changeStreamStageOptions.startAtClusterTime = { ts };
  }
